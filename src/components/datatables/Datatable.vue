...
<v-col>
  ...
  >
    ...
    <template v-slot:item.cantidad_dias_desagrupada="{ item }">
        <v-chip :color="getColor(item.cantidad_dias_desagrupada)" dark>{{ item.cantidad_dias_desagrupada }}</v-chip>
    </template>
...
export default {
  ...
  methods:{
    ...mapMutations(['mostrarNotificacion']),
      getColor (valor) {
        if (valor >= 17) return 'red'
        else if (valor > 11 && valor <17) return 'orange'
        else return 'green'
      },
      getColorGuias (valor) {
        if (valor >= 7) return 'red'
        else if (valor >= 5 && valor < 7) return 'orange'
        else return 'green'
      },
      exportar(){
        alert(this.items[0].codigo)
      } 
  }
}
</script>
